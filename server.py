...
def handle_connect():
    if current_user.is_authenticated:
        ...
        print(f"DIAG: User connected: {current_user.username}. Online users: {online_users}")
        unread_messages = db.session.query(Message.sender_id, db.func.count(Message.id)).filter(Message.recipient_id == current_user.id, Message.is_read == False).group_by(Message.sender_id).all()
        if unread_messages:
...
def handle_disconnect():
    if current_user.is_authenticated and current_user.id in online_users:
        ...
        print(f"DIAG: User disconnected: {current_user.username}. Online users: {online_users}")
...
def handle_private_message(data):
    ...
    payload = {'id': new_message.id, 'sender': current_user.username, 'message': message_content, 'is_read': False}
    recipient_sid = online_users.get(recipient_id); sender_sid = online_users.get(current_user.id)
    print(f"DIAG: Handling message from {current_user.username} (sid: {sender_sid}) to user_id {recipient_id} (sid: {recipient_sid})")
    if recipient_sid:
        print(f"DIAG: Emitting 'message_from_server' to recipient {recipient_id}")
        emit('message_from_server', payload, room=recipient_sid)
        if sender_sid: emit('message_status_update', {'id': new_message.id, 'status': 'Đã nhận'}, room=sender_sid)
    else: print(f"DIAG: Recipient {recipient_id} is offline.")
    if sender_sid: print(f"DIAG: Echoing 'message_from_server' to sender {current_user.username}"); emit('message_from_server', payload, room=sender_sid)

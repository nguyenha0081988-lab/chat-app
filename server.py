...
def check_for_update():
    ...
    # Lấy phiên bản mới nhất dựa trên timestamp
    ...
    # Trả về phiên bản hiện tại (hoặc 0.0.0) nếu chưa có bản ghi nào
    return jsonify({
...
# ... (Giữ nguyên các route khác: /, /login, /online-users, /history, /delete-file, /upload, /files, /download, /avatars, /avatar/upload, /avatar/select, /admin/users) ...
...
def upload_file():
    ...
    try:
        upload_result = cloudinary.uploader.upload(file, folder=CLOUDINARY_FOLDER, resource_type="auto")
...
@login_required
def get_files():
    files = File.query.all(); file_list = [{'filename': f.filename, 'public_id': f.public_id} for f in files]
...
def get_user_avatars():
    user_files = File.query.filter((File.user_id == current_user.id)).all(); avatars = []
    for f in user_files:

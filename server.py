...
def get_history(partner_id):
    # API này bây giờ cũng sẽ đánh dấu tin nhắn là đã đọc
    messages_to_mark_read = db.session.query(Message).filter((Message.sender_id == partner_id) & (Message.recipient_id == current_user.id))
...
def handle_private_message(data):
    ...
    if recipient_sid: 
        ...
        # Báo cho người gửi rằng tin nhắn đã được "Đã nhận"
    ...
    emit('message_from_server', payload, room=request.sid)

@socketio.on('mark_as_read')
def handle_mark_as_read(data):
    partner_id = data['partner_id']
    messages_to_mark = db.session.query(Message).filter(Message.sender_id == partner_id, Message.recipient_id == current_user.id, Message.is_read == False)
    message_ids = [msg.id for msg in messages_to_mark.all()]
    messages_to_mark.update({Message.is_read: True}); db.session.commit()
    
    partner_sid = online_users.get(partner_id)
    if partner_sid:
        emit('messages_seen', {'ids': message_ids, 'viewer_username': current_user.username}, room=partner_sid)
...
# ... (Các hàm và phần còn lại giữ nguyên)
